...
def put_settings(userId: str, payload: dict, user_id: str = Depends(get_user_id)):
    if userId != user_id:
        raise HTTPException(status_code=403, detail="Forbidden")
    ...
    if not primary_color or not validate_hex_color(primary_color):
        raise HTTPException(status_code=400, detail="Invalid primary color")
...
def upload_logo(userId: str, file: UploadFile = File(...), user_id: str = Depends(get_user_id)):
    ...
    url = upload_gcs_file_from_stream(file.file, filename, bucket_name)
    return {"logoUrl": url}
